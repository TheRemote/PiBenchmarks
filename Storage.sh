...
if [[ -n "$(which apt)" ]]; then
  ...
  apt-get install lshw pciutils usbutils lsscsi bc curl hwinfo hdparm nvme-cli dmidecode smartmontools fio  -y
  ...
  if [ -z "$(which smartctl)" ]; then apt-get install smartmontools -y; fi
